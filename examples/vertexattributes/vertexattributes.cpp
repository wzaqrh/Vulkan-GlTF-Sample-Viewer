...
/*
 * This sample demonstrates different ways of passing vertex data to the gpu: 
 * As one interleaved buffer (with all attributes) and as separate buffers per attribute
 */
...
void VulkanExample::render()
{
	...
	vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vertexAttributeSettings == VertexAttributeSettings::separate ? pipelines.vertexAttributesSeparate : pipelines.vertexAttributesInterleaved);
	...
	if (vertexAttributeSettings == VertexAttributeSettings::separate) {
		...
		vkCmdBindVertexBuffers(commandBuffer, 0, 1, &interleavedVertexBuffer.buffer, offsets);
	}
